...
public class Bank_소현2 {
	public static void main(String[] args) {
		...
		for (;;) {
			...
			if (sel == 9) { System.out.println("종료 기능입니다."); break; }  
			//##  ctrl + alt + j  한줄    -> break / return
			if ( (sel<0) || (sel>5)) { System.out.println("다시 확인해 주세요."); continue; }  
			...
			if (sel == 1 ) {
				...
				for(int i =0; i<id.length;i++) { // 아이디가 있는지 배열을 반복해서 확인
					if (id[i].equals(tempId)  && pw[i].equals(tempPw)  ) { // 아이디가 널값이 아니면 비교
						found = i;  
					}
				}
				if (found==-1) {
